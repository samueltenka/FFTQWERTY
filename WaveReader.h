...
void read_wave(const char* filename, real_1d_array &x);
void read_wav_header(FILE* myfile, unsigned int *samp_rate, unsigned int *bits_per_samp, unsigned int *num_samp);
void read_wav_data(FILE* myfile, double* x, unsigned int samp_rate, unsigned int bits_per_samp, unsigned int num_samp);
...
void read_wav_data(FILE* myfile, double* x, unsigned int samp_rate, unsigned int bits_per_samp, unsigned int num_samp)
{
   ...
   for (i=0; i < num_samp; ++i) {
		...
		x[i] = static_cast<double>(conv_bit_size(tmp, bits_per_samp));
   }
}
...
//void read_wave(const char* filename, real_1d_array &x);
//void read_wav_header(FILE* myfile, unsigned int *samp_rate, unsigned int *bits_per_samp, unsigned int *num_samp);
//void read_wav_data(FILE* myfile, real_1d_array &x, unsigned int samp_rate, unsigned int bits_per_samp, unsigned int num_samp);
//int conv_bit_size(unsigned int in, int bps);
//
///*THX TO http://ubuntuforums.org/showthread.php?t=960693 */
///*e.g.:
...
//	printf("samp_rate=[%d] bits_per_samp=[%d] num_samp=[%d]\n",
//		     samp_rate, bits_per_samp, num_samp);
//   x.setlength(num_samp);
//	//int *data = (int *) malloc(num_samp * sizeof(int));
//   read_wav_data(myfile, x, samp_rate, bits_per_samp, num_samp);
//
//   fclose(myfile);
//}
//
